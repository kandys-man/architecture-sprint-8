...
1. Повторите теорию, которая пригодится:
 - OAuth 2.0.
 - JWT.
 - PKCE Code grant.
 - RBAC.
...
3. Подготовьте репозиторий, куда вы будете загружать решения заданий. Для этого склонируйте репозиторий по ссылке https://github.com/Yandex-Practicum/architecture-sprint-8. 
   В коде уже есть изменения, которые вы вносили в уроках.
...
2. **Создайте бэкенд-часть приложения для API.** Выберите удобный для вас язык — Pyhton, Java, C# или любой другой. 
   Добавьте API `/reports` в этот бэкенд для передачи отчётов. Тут не требуется поход в базы данных, ограничьтесь генерацией произвольных данных. 
   Реализация сбора фактических данных будет в следующем задании.

Перед отправкой задания проверьте, что:

1. Бэкенд отдаёт данные только пользователям с ролью `prothetic_user`. Эту роль мы добавили в последнем уроке второй темы `Реализация Keycloak: практика`.
